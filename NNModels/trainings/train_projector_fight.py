...
from train_common import load_database, load_features, save_network, save_network_onnx, load_latent
...
if __name__ == '__main__':
    ...
    X = load_features('./generate/data/fight/{0}/features.bin'.format(ms.settings_animation_type))['features'].copy().astype(np.float32)
    Z = load_latent('./train_ris/fight/{0}/decompressor/latent.bin'.format(ms.settings_animation_type))['latent'].copy().astype(np.float32)
    ...
    def generate_predictions():
        with torch.no_grad():
            ...
            stop = min(start + 200, range_stops[0])
            ...
            try:
                plt.savefig('./train_ris/fight/{0}/projector/projector_X.png'.format(ms.settings_animation_type))
            except IOError as e:
                print(e)
            ...
            try:
                plt.savefig('./train_ris/fight/{0}/projector/projector_Z.png'.format(ms.settings_animation_type))
            except IOError as e:
                print(e)
    ...
    for i in range(niter):
        ...
        if i % 10000 == 0:
            ...
            save_network('./train_ris/fight/{0}/projector/projector.bin'.format(ms.settings_animation_type), [
                projector.layer1,
                projector.layer2,
                projector.layer3,
                projector.layer4,
            ...
            save_network_onnx(projector,
                              projector_mean_in,
                              './train_ris/fight/{0}/projector/projector.onnx'.format(ms.settings_animation_type))
