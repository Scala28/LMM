...
if __name__ == '__main__':
    # Load data
    database = load_database('./generate/data/terrain/{0}/database.bin'.format(ms.animation_type))
    ...
    X = load_features('./generate/data/terrain/{0}/features.bin'.format(ms.animation_type))['features'].astype(np.float32)
    ...
    def _save_compressed_database():
        with torch.no_grad():
            ...
            with open('./train_ris/terrain/{0}/decompressor/latent.bin'.format(ms.animation_type), 'wb') as f:
    ...
    def _generate_anim():
        with torch.no_grad():
            ...
            # Write BVH
            try:
                Bvh.save('./train_ris/terrain/{0}/decompressor/decompressor_Ygnd.bvh'.format(ms.animation_type), {
                ...
                Bvh.save('./train_ris/terrain/{0}/decompressor/decompressor_Ytil.bvh'.format(ms.animation_type), {
    ...
    for i in range(niter):
        ...
        if i % 10000 == 0:
            ...
            save_network('./train_ris/terrain/{0}/decompressor/decompressor.bin'.format(ms.animation_type), [
                decompressor.layer1,
            ...
            save_network_onnx(decompressor,
                              decompressor_mean_in,
                              './train_ris/terrain/{0}/decompressor/decompressor.onnx'.format(ms.animation_type))
