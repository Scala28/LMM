...
nfeatures = 3 + 3 + 3 + 3 + 3 + 6 + 6 + 4 + 4 + (1 if ms.animation_type == 'actions' else 0)

features = np.zeros((nframes, nfeatures))
features_offset = np.zeros(nfeatures)
features_scale = np.zeros(nfeatures)

print("Writing features generate/data/terrain/{0}/feature.bin ...".format(ms.animation_type))
...
with open('generate/data/terrain/{0}/features.bin'.format(ms.animation_type), 'wb') as f:
