...
def parse_hierarchy(lines):
    ...
    for i, line in enumerate(lines):
        ...
        if trimmed.startswith("ROOT") or trimmed.startswith("JOINT"):
            ...
            fingers_list = ["HandPinky", "HandIndex", "HandRing", "HandMiddle", "HandThumb"]
            if any(word in joint_name for word in fingers_list):
                remove_mode = True
...
def remove_repeated_brace_couplets(lines):
    """Remove repeated closing brace groups of 2 instead of 4."""
    cleaned = []
    i = 0
    while i < len(lines):
        # Check if there are at least two consecutive closing braces
        if i + 1 < len(lines) and is_closing_brace_line(lines[i]) and is_closing_brace_line(lines[i + 1]):
            base_lines = [lines[i].rstrip('\n'), lines[i + 1].rstrip('\n')]
            j = i
            repetition_count = 0

            # Detect repeated couplets of `}`
            while j + 1 < len(lines) and is_closing_brace_line(lines[j]) and is_closing_brace_line(lines[j + 1]) \
                    and lines[j].rstrip('\n') == base_lines[0] and lines[j + 1].rstrip('\n') == base_lines[1]:
                repetition_count += 1
                j += 2  # Move forward by 2 lines

            if repetition_count > 1:
                i = j  # Skip repeated blocks
                continue
            else:
                cleaned.extend(lines[i:i + 2])
                i += 2
                continue
        else:
...
def add_toe_joints(lines):
    """
    Find 'JOINT LeftFoot' and 'JOINT RightFoot' lines, keep the next 6 lines after them,
    and then append new toe joint definitions with the correct structure and indentation.

    Returns:
    - new_lines: Modified hierarchy lines
    - toe_indices: Indices where new toe joints were added (for updating motion data)
    """
    new_lines = []
    toe_indices = []  # Stores indices of new toe joints for motion updates
    i = 0
    ...
    while i < len(lines):
        ...
        # Check if the line defines LeftFoot or RightFoot
        if stripped_line.startswith("JOINT LeftFoot") or stripped_line.startswith("JOINT RightFoot"):
            new_lines.append(line)  # Keep the JOINT LeftFoot/RightFoot line
            ...
            i += 1
            if "Right" in stripped_line:
                new_lines.append(f"{indent}\tJOINT RightToe\n")
                toe_indices.append(joint_index + 1)  # Store RightToe index for motion
            if "Left" in stripped_line:
                new_lines.append(f"{indent}\tJOINT LeftToe\n")
                toe_indices.append(joint_index + 1)  # Store LeftToe index for motion

            for _ in range(2):
                i += 1
                new_lines.append(lines[i])

            # Add the additional toe joint structure at the same indentation level
            new_lines.append(f"{indent}\t\tCHANNELS 3 Yrotation Xrotation Zrotation\n")
            new_lines.append(f"{indent}\t\tEnd Site\n")
            new_lines.append(f"{indent}\t\t{{\n")
            new_lines.append(f"{indent}\t\t\tOFFSET 0.000 10.000 0.000\n")
            new_lines.append(f"{indent}\t\t}}\n")
        else:
            new_lines.append(line)
    ...
    toe_indices[0] += 1
    toe_indices[1] += 2
    return new_lines, toe_indices


def update_motion_for_toes(lines, toe_indices, joint_channel_counts):
    """
    Adds motion data for the newly inserted toe joints.

    - The new joints have 3 channels, so we insert three `0.000` values at the correct positions.

    Parameters:
    - lines: Original motion section lines
    - toe_indices: Indices where toe joints were added (from add_toe_joints)
    - joint_channel_counts: List containing the number of channels per joint

    Returns:
    - new_lines: Updated motion data with the additional 0.000 values for new toe joints
    """
    new_lines = []
    for i, line in enumerate(lines):
        if i < 3:  # First two lines contain 'MOTION' and 'Frames:' (keep them unchanged)
            new_lines.append(line)
            continue
        ...
        column_index = 0  # Track the current column index in motion data
        toe_inserted = 0  # Track how many toes we've added so far

        for joint_idx, num_channels in enumerate(joint_channel_counts):
            if joint_idx in toe_indices:
                # Insert the new 3 channels (0.000) before this joint's motion data
                updated_values.extend(["0.000", "0.000", "0.000"])
                toe_inserted += 1
            # Copy the original motion data for this joint
            updated_values.extend(values[column_index: column_index + num_channels])
            column_index += num_channels

        new_lines.append(" ".join(updated_values) + "\n")
...
def process_bvh(input_bvh, output_bvh, add_toe=False):
    ...
    if add_toe:
        new_hierarchy, toe_joint_indices = add_toe_joints(new_hierarchy)

    # Process motion
    new_motion = filter_motion(motion_lines, removed_indices, joint_channel_counts)
    print("hands joints removed")
    if add_toe:
        new_motion = update_motion_for_toes(new_motion, toe_joint_indices, joint_channel_counts)
        print("toe joints added")
